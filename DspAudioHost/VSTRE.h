#pragma once
// This was created from released VST2.x plugins, and is technically under the 2-clause
// BSD license. Depending on which country you are in, Steinberg can do fuck all about
// this. Notable countries for this are most members of the United States of America, the
// entirety of Europe, Japan, and Russia. Consult a lawyer if you don't know if clean room
// reverse engineering is allowed in your country.
...
#define VST_MAX_CHANNELS                                                                 \
    32 // Couldn't find any audio editing software which would attempt to add more
       // channels.
...
enum VST_EFFECT_OPCODE {
     ...
     *
     * @param p_float New sample rate as a float (double on 64-bit because register
     * upgrades).
     */
    ...
    /* Sets the block size, which is the maximum number of samples passed into the effect
     * via process calls.
     *
     ...
     * @param p_ptr Pointer to a char* buffer able to hold at minimum 20 characters. Might
     * need to be 32 even.
     ...
     *
     * @param p_int2 (vst_speaker_arrangement*) Pointer to a pointer to the speaker
     * arrangement for the input.
     * @param p_ptr (vst_speaker_arrangement*) Pointer to a pointer to the speaker
     ...
     *
     * @param p_ptr char[64] Buffer containing a zero-terminated effect information
     * string. May be shorter than 64 bytes on older hosts.
     ...
     *
     * @param p_ptr char[64] Buffer containing a zero-terminated vendor information
     * string. May be shorter than 64 bytes on older hosts.
     * @return Always 0, even on failure.
     */
     ...
     *
     * @return Return 0 for <2.0, 2 for 2.0, 2100 for 2.1, 2200 for 2.2, 2300 for 2.3,
     * etc.
     */
...
struct vst_effect {
     ...
     */
    intptr_t(VST_FUNCTION_INTERFACE* control)(vst_effect* pthis, VST_EFFECT_OPCODE opcode,
        int32_t p_int1, intptr_t p_int2, void* p_ptr, float p_float);
     ...
     */
    void(VST_FUNCTION_INTERFACE* process)(
        vst_effect* pthis, const float* const* inputs, float** outputs, int32_t samples);

    /* Updates the value for the parameter at the given index, or does nothing if out of
     * bounds.
     *
     * @param pthis Pointer to the effect itself.
     ...
     */
    void(VST_FUNCTION_INTERFACE* set_parameter)(
        vst_effect* pthis, uint32_t index, float value);
     ...
     * 5		Has process_float (Found in: ReaDelay, ReaComp, ReaControlMIDI, ReaStream,
     * ReaFir) 6		Unknown (Found in: ReaControlMIDI, ReaStream, ReaFir) 10
     * Unknown (Found in: ReaFir) 13		Has process_double (Found in: ReaControlMIDI)
     */
    int32_t flags;
     ...
     * @param samples Number of samples per channel in inputs.
     */
    void(VST_FUNCTION_INTERFACE* process_float)(
        vst_effect* pthis, const float* const* inputs, float** outputs, int32_t samples);
     ...
     * @param samples Number of samples per channel in inputs.
     */
    void(VST_FUNCTION_INTERFACE* process_double)(vst_effect* pthis,
        const double* const* inputs, double** outputs, int32_t samples);
...
struct vst_speaker_arrangement {
    ...
    vst_speaker_properties speakers[VST_MAX_CHANNELS]; // Array of speaker properties,
                                                       // actual size defined by channels.
};
 ...
 * @return ?
 */
typedef intptr_t (*vst_host_callback)(vst_effect* plugin, VST_HOST_OPCODE opcode,
    int32_t p_int1, int64_t p_int2, const char* p_str, int32_t p_int3);

#define VSTCALLBACK vst_host_callback
...
#ifdef __cplusplus
template <size_t T> struct vst_speaker_arrangement_t {
    VST_ARRANGEMENT_TYPE type; // See VST_SPEAKER_ARRANGEMENT_TYPE
    int32_t channels; // Number of channels in speakers.
    vst_speaker_properties
        speakers[T]; // Array of speaker properties, actual size defined by channels.
};
#endif
