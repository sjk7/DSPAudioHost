        ...
        while (cr == ERROR_SUCCESS && !found) {
            ...
            if (cr == ERROR_SUCCESS) {
                ...
                if (!(strcmp(databuf, clsidstr))) {
                    if ((cr = RegOpenKeyEx(hkEnum, (LPCTSTR)databuf, 0, KEY_READ, &hksub))
                        == ERROR_SUCCESS) {
                        if ((cr = RegOpenKeyEx(
                                 hksub, (LPCTSTR)INPROC_SERVER, 0, KEY_READ, &hkpath))
                            == ERROR_SUCCESS) {
                            datatype = REG_SZ;
                            datasize = (DWORD)dllpathsize;
                            cr = RegQueryValueEx(
                                hkpath, 0, 0, &datatype, (LPBYTE)dllpath, &datasize);
                            if (cr == ERROR_SUCCESS) {
            ...
            if (cr == ERROR_SUCCESS) {
                ...
                if (rc == 0) {
                    ...
                    if (lpdrv) {
                        ...
                        if ((cr = CLSIDFromString((LPOLESTR)wData, (LPCLSID)&clsid))
                            == S_OK) {
